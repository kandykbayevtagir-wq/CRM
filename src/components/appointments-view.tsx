...
import { CalendarDays, Check, Download, Plus, Search } from "lucide-react";
  ...
  return (
    <>
      <PageHeader
        ...
        actions={<><Button variant="secondary" onClick={() => window.print()}><Download size={15} /> Печать</Button><Button onClick={() => { setFormError(null); setModalOpen(true); }}><Plus size={16} /> Новая запись</Button></>}
      />
              ...
              <table className="data-table">
                ...
                <tbody>{items.map((appointment, index) => {
                  ...
                  return <tr key={appointment.id}>
                    ...
                    <td>{!["completed", "cancelled", "no_show"].includes(currentStatus) ? <button className="inline-action" onClick={() => void completeAppointment(appointment.id)} disabled={updatingId === appointment.id} title="Завершить приём"><Check size={14} /> {updatingId === appointment.id ? "…" : "Завершить"}</button> : null}</td>
                  </tr>;
                })}</tbody>
              </table>
            </div>
