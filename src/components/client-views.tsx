...
export function ClientBookingView() {
  ...
  useEffect(() => { setSelectedSlot(null); setEmployeeOptions([]); setIdempotencyKey(""); }, [branchId, serviceId, date]);
  useEffect(() => { setSelectedSlot(null); setIdempotencyKey(""); }, [employeeId]);
  useEffect(() => {
  ...
  async function book() {
    ...
    try {
      const response = await apiFetch<{ ok: true; id: string; changed: boolean }>("/api/client/appointments", { method: "POST", body: { startsAt: selectedSlot.startsAt, serviceId, branchId, employeeId: selectedSlot.employeeId, idempotencyKey: requestKey, ...(rescheduleId ? { appointmentId: rescheduleId } : {}) } });
