...
export const onRequestPost: PagesFunction<CrmEnv> = async ({ request, env, params }) => {
  ...
  for (const item of items) {
    ...
    statements.push(
      env.DB.prepare("UPDATE purchase_items SET received_quantity = received_quantity + ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(item.quantity, item.purchaseItemId),
      env.DB.prepare("INSERT INTO stock_movements (id, product_id, branch_id, movement_type, direction, quantity, unit_price, total_cost, occurred_at, user_id, source, purchase_id, purchase_item_id, idempotency_key, comment) VALUES (?, ?, ?, 'PURCHASE', 'IN', ?, ?, ?, ?, ?, 'PURCHASE_RECEIPT', ?, ?, ?, ?)").bind(movementId, row.productId, purchase.branchId, item.quantity, unitCost, totalCost, dateValue(body, "receivedAt") || new Date().toISOString(), user.id, purchaseId, item.purchaseItemId, movementKey, `Приёмка закупки ${purchaseId}`),
      env.DB.prepare("UPDATE products SET purchase_price = ?, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(unitCost, row.productId),
    );
  }
