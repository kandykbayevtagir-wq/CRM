...
export const onRequestGet: PagesFunction<CrmEnv> = async ({ request, env }) => {
  ...
  const [services, branches, profile] = await Promise.all([
    env.DB.prepare("SELECT id, name, category, price, cost, duration_minutes AS durationMinutes, is_active AS isActive FROM services WHERE is_active = 1 ORDER BY category ASC, name ASC").all(),
