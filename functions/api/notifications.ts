...
export const onRequestGet: PagesFunction<CrmEnv> = async ({ request, env }) => {
  ...
  const [explicit, audit, upcoming, obligations, lowStock, overdueTasks, followUps, inventoryIssues, failedOutbox, unpaidAppointments] = await Promise.all([
    ...
    env.DB.prepare(`SELECT p.id, p.name, COALESCE(SUM(CASE WHEN sm.direction = 'IN' THEN sm.quantity ELSE -sm.quantity END), 0) AS currentStock, p.min_stock AS minStock FROM products p LEFT JOIN stock_movements sm ON sm.product_id = p.id WHERE p.is_active = 1 GROUP BY p.id HAVING currentStock <= p.min_stock ORDER BY p.name LIMIT 8`).all<{ id: string; name: string; currentStock: number; minStock: number }>(),
    env.DB.prepare("SELECT id, title, due_date AS dueDate FROM tasks WHERE status IN ('OPEN', 'IN_PROGRESS') AND due_date < CURRENT_TIMESTAMP ORDER BY due_date LIMIT 8").all<{ id: string; title: string; dueDate: string }>(),
    env.DB.prepare("SELECT f.id, f.recommended_date AS recommendedDate, c.full_name AS clientName FROM follow_ups f INNER JOIN clients c ON c.id = f.client_id WHERE f.status = 'OPEN' AND f.recommended_date <= date('now', '+7 days') ORDER BY f.recommended_date LIMIT 8").all<{ id: string; recommendedDate: string; clientName: string }>(),
    env.DB.prepare("SELECT id, message, created_at AS createdAt FROM inventory_issues WHERE status = 'OPEN' ORDER BY created_at DESC LIMIT 8").all<{ id: string; message: string; createdAt: string }>(),
    env.DB.prepare("SELECT id, last_error AS lastError, updated_at AS updatedAt FROM message_outbox WHERE status = 'FAILED' ORDER BY updated_at DESC LIMIT 8").all<{ id: string; lastError: string | null; updatedAt: string }>(),
    env.DB.prepare(`SELECT a.id, a.starts_at AS startsAt, c.full_name AS clientName, a.total_amount AS totalAmount, COALESCE((SELECT SUM(p.amount) FROM payments p WHERE p.appointment_id = a.id AND p.payment_status = 'POSTED'), 0) - COALESCE((SELECT SUM(pa.amount) FROM payment_adjustments pa INNER JOIN payments rp ON rp.id = pa.payment_id WHERE rp.appointment_id = a.id), 0) AS paidAmount FROM appointments a INNER JOIN clients c ON c.id = a.client_id WHERE a.status = 'COMPLETED' AND a.total_amount > (COALESCE((SELECT SUM(p.amount) FROM payments p WHERE p.appointment_id = a.id AND p.payment_status = 'POSTED'), 0) - COALESCE((SELECT SUM(pa.amount) FROM payment_adjustments pa INNER JOIN payments rp ON rp.id = pa.payment_id WHERE rp.appointment_id = a.id), 0)) ORDER BY a.starts_at DESC LIMIT 8`).all<{ id: string; startsAt: string; clientName: string; totalAmount: number; paidAmount: number }>(),
  ]);
