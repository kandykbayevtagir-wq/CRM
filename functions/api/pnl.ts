...
export async function metricSnapshot(db: D1Database, from: string, to: string, request: Request) {
  ...
  const [revenue, refunds, consumables, expenses, payroll, counts, lines, daily, expenseGroups] = await Promise.all([
    ...
    db.prepare(`SELECT COALESCE(SUM(l.total_amount), 0) AS value FROM payroll_lines l INNER JOIN payroll_periods pp ON pp.id = l.period_id WHERE pp.status IN ('CALCULATED', 'CLOSED') AND pp.period_start < ? AND pp.period_end >= ?`).bind(to, from).first<{ value: number }>(),
    ...
    db.prepare(`SELECT a.id, a.starts_at AS startsAt, a.employee_id AS employeeId, e.full_name AS employeeName, a.branch_id AS branchId, s.id AS serviceId, s.name AS serviceName, s.category, aps.price, aps.quantity, a.total_amount AS appointmentAmount,
      ...
      COALESCE((SELECT SUM(pa.amount) FROM payment_adjustments pa INNER JOIN payments rp ON rp.id = pa.payment_id WHERE rp.appointment_id = a.id), 0) AS refundedAmount,
      ...
      FROM appointments a INNER JOIN appointment_services aps ON aps.appointment_id = a.id INNER JOIN services s ON s.id = aps.service_id LEFT JOIN employees e ON e.id = a.employee_id WHERE a.status = 'COMPLETED' AND ${where}`).bind(from, to, ...scope.bindings).all<Record<string, unknown>>(),
    db.prepare(`SELECT date(p.paid_at, 'localtime') AS day, COALESCE(SUM(p.amount), 0) AS amount FROM payments p INNER JOIN appointments a ON a.id = p.appointment_id WHERE a.status = 'COMPLETED' AND p.payment_status = 'POSTED' AND p.paid_at >= ? AND p.paid_at < ? AND ${where} GROUP BY date(p.paid_at, 'localtime') ORDER BY day`).bind(from, to, ...scope.bindings).all<{ day: string; amount: number }>(),
    db.prepare(`SELECT x.kind AS category, COALESCE(SUM(x.amount), 0) AS amount FROM financial_transactions x WHERE x.status = 'POSTED' AND x.direction = 'EXPENSE' AND x.kind NOT IN ('SALARY', 'RENT', 'UTILITIES') AND x.occurred_at >= ? AND x.occurred_at < ?${scope.branchId ? " AND x.branch_id = ?" : ""} GROUP BY x.kind ORDER BY amount DESC`).bind(from, to, ...(scope.branchId ? [scope.branchId] : [])).all<{ category: string; amount: number }>(),
  ]);
