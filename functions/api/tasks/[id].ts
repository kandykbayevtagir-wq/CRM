...
export const onRequestPatch: PagesFunction<CrmEnv> = async ({ request, env, params }) => {
  ...
  if (user.role === "SPECIALIST" && existing.assignee_id && existing.assignee_id !== user.id) return forbidden();
  ...
  await env.DB.batch([
    env.DB.prepare("UPDATE tasks SET title = ?, description = ?, assignee_id = ?, due_date = ?, priority = ?, status = ?, completed_at = CASE WHEN ? = 'DONE' THEN COALESCE(completed_at, CURRENT_TIMESTAMP) ELSE NULL END, updated_at = CURRENT_TIMESTAMP WHERE id = ?").bind(stringValue(body, "title", String(existing.title ?? "")), optionalString(body, "description") ?? existing.description ?? null, optionalString(body, "assigneeId") ?? existing.assignee_id ?? null, dateValue(body, "dueDate") || existing.due_date || null, priority, status, status, id),
