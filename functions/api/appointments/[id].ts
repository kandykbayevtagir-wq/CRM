...
export const onRequestPatch: PagesFunction<CrmEnv> = async ({ request, env, params }) => {
  ...
  if (user.role === "SPECIALIST") {
    const own = await env.DB.prepare("SELECT id FROM employees WHERE user_id = ? AND id = ?").bind(user.id, String(existing.employee_id ?? "")).first();
    if (!own) return forbidden("Специалист может изменять только свои записи");
  }
